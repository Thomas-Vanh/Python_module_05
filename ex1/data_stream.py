...
class DataStream():
    ...
    def process_stream(self, stream: list[typing.Any]) -> None:
        for element in stream:
            ...
            for item in self._proc:
                self._processor: DataProcessor = item["instance"]
                if self._processor.validate(element):
                    self._processor.ingest(element)
                    item["total"] += 1
                    handled = True
                    break
            if not handled:
    ...
    def print_processors_stats(self) -> None:
        if not self._processor:
            ...
            return
        name: str = self._processor.__class__.__name__
        rem_proc: int = len(self._processor._lst)
        print("== DataStream statistics ==")
        print(f"{name}: total {len(self._processor._lst)} items processed,"
